...
Yes. There actually is, but it really depends on how you set up your data for modeling. For example, if your detection history represents weeks of sampling instead of days of sampling, then you may want to account for the fact that camera traps were not 100% operational at sites each week. In this case, you could generate a survey-specific covariate that represents the proportion of a week that the given camera was active. This would help control for the fact that a camera that was only operational for 1 of 7 days will likely have a different detection probability than a camera that was operational for 7 of 7 days. What you do not want to do, however, is include a covariate that represents 'the total number of weeks sampled across a primary sampling period.' If the covariate represents something survey specific, then you are good to go. If the covariate represents sampling effort overall, then do not include it within the detection probability of your model.

In closing, it is important to carefully consider why you are doing something and not just listen to some person (e.g., me) tell you that 'it must be done in this way.' More often then not, including the number of survey days in the detection part of the model does not make a lot of sense even though you will often find significant differences with it. But if you have a valid reason for it, just make sure you justify it and provide some logic behind why you are doing it! It's as 'simple' as that.

EDIT: Thanks to Lisanne Petracca for suggesting I add in how to actually control for variation in survey effort within an occupancy model.
